...
public abstract class CollectionResultTExtensionTestBase
{
    ...
    protected readonly List<Result<int>> MixedCollection = [1, new TestFailure("code1"), 3, new TestFailure("code2")];
    ...
    protected readonly List<Task<Result<int>>> MixedTaskCollection = [Result.SuccessTask(1), Task.FromResult(Result<int>.Failure(new TestFailure("code1"))),
        Result.SuccessTask(3), Task.FromResult(Result<int>.Failure(new TestFailure("code2")))];
