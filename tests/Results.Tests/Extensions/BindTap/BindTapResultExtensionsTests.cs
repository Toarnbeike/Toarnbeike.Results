...
public class BindTapResultExtensionsTests
{
    ...
    private readonly Func<Result> _failureFunc = () => Result.Failure(new TestFailure("BindTap"));
    ...
    private readonly Func<Result<int>> _failureOfTFunc = () => Result<int>.Failure(new TestFailure("BindTap"));
    ...
    private readonly Func<Task<Result>> _failureTaskFunc = () => Task.FromResult(Result.Failure(new TestFailure("BindTap")));
    ...
    private readonly Func<Task<Result<int>>> _failureOfTTaskFunc = () => Task.FromResult(Result<int>.Failure(new TestFailure("BindTap")));
