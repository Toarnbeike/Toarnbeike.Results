...
public class ZipExtensionsTests
{
    ...
    private readonly Func<double, Result<int>> _secondFailureFunc = _ => Result<int>.Failure(new TestFailure("zip"));
    ...
    private readonly Func<double, Task<Result<int>>> _secondFailureAsyncFunc = _ => Task.FromResult(Result<int>.Failure(new TestFailure("zip")));
    ...
    public void Zip_Should_ReturnSecondFailure_WhenFirstResultIsSuccess_AndSecondResultIsFailure()
    {
        ...
        result.ShouldBeFailure().Message.ShouldBe("zip");
    }
